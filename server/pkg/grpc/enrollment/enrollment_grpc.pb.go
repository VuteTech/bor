...
const (
	...
	EnrollmentService_KerberosEnroll_FullMethodName        = "/bor.enrollment.v1.EnrollmentService/KerberosEnroll"
)
...
type EnrollmentServiceClient interface {
	...
	// No manually-generated enrollment token is required; the agent authenticates
	// using its machine keytab (/etc/krb5.keytab).  Supported by FreeIPA and
	// Active Directory (Samba AD) domains.
	KerberosEnroll(ctx context.Context, in *KerberosEnrollRequest, opts ...grpc.CallOption) (*EnrollResponse, error)
}
...
func (c *enrollmentServiceClient) KerberosEnroll(ctx context.Context, in *KerberosEnrollRequest, opts ...grpc.CallOption) (*EnrollResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(EnrollResponse)
	err := c.cc.Invoke(ctx, EnrollmentService_KerberosEnroll_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type EnrollmentServiceServer interface {
	...
	// using its machine keytab (/etc/krb5.keytab).  Supported by FreeIPA and
	// Active Directory (Samba AD) domains.
	KerberosEnroll(context.Context, *KerberosEnrollRequest) (*EnrollResponse, error)
...
func (UnimplementedEnrollmentServiceServer) KerberosEnroll(context.Context, *KerberosEnrollRequest) (*EnrollResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method KerberosEnroll not implemented")
}
...
func _EnrollmentService_KerberosEnroll_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(KerberosEnrollRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(EnrollmentServiceServer).KerberosEnroll(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: EnrollmentService_KerberosEnroll_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(EnrollmentServiceServer).KerberosEnroll(ctx, req.(*KerberosEnrollRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var EnrollmentService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "KerberosEnroll",
			Handler:    _EnrollmentService_KerberosEnroll_Handler,
		},
	},
